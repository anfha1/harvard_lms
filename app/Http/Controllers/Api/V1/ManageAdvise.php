...
class ManageAdvise extends Controller
{
            ...
            if ($info['status']) {
                if (App::auth($info['info_user'], [1, 2])) {
                    ...
                    if ($next) {
                        ...
                        $list_role = ladvise_session_role::select('ladvise_session_id')->where('luser_id', $user_select)->get();
                        ...
                        $list_course = ladvise::select('id', 'name')->get();
                        $list_course_group = [];
                        $list_role = [];
                        if ($list_course->count() > 0) {
                            ...
                            $list_session = ladvise_session::select('id', 'name', 'ladvise_id')->get();
                            if ($list_session->count() > 0) {
                                foreach ($list_session as $session) {
                                    if (isset($list_course_group[(int)$session->ladvise_id])) {
                                        $list_course_group[(int)$session->ladvise_id]['sessions'][] = [
                                            'id' => $session->id,
                                            'name' => $session->name,
                                        ];
                                    }
                ...
                if ($info['status']) {
                    if (App::auth($info['info_user'], [1, 2])) {
                        ...
                        if ($next) {
                            ...
                            // lấy danh sách quyền người dùng đó đang có
                            $list_role = ladvise_session_role::select('id', 'ladvise_session_id')->where('luser_id', $user_select)->get();

                            foreach ($list_role as $role) {
                                if (in_array($role->ladvise_session_id, $list_check)) {
                                    // xóa giá trị khỏi $list_check
                                    $list_av[] = $role->ladvise_session_id;
                                    if (($key = array_search($role->ladvise_session_id, $list_check)) !== false) {
                            ...
                            if (count($list_check) > 0) {
                                $notTrue = false;
                                ladvise_session_role::insert(array_map(function($id) use ($user_select) {
                                    return [
                                        'luser_id' => $user_select,
                                        'ladvise_session_id' => $id,
                                    ];
                                }, $list_check));
                            }
                            ...
                            if (count($list_del)) {
                                $notTrue = false;
                                ladvise_session_role::whereIn('id', $list_del)->delete();
                            }
    ...
    public function edit(Request $request) {
        ...
        if (Validate::number($res, $request_all, 'course_id', 'Khối (lớp)')) {
            ...
            if ($info['status']) {
                if (App::auth($info['info_user'], 1)) {
                    $course = ladvise::find($request_all['course_id']);
                    if ($course) {
                        $change = false;
            ...
            if ($info['status']) {
                if (App::auth($info['info_user'], 1)) {
                    ...
                    if ($course) {
                        ...
                        ladvise_session::where('ladvise_id', $request_all['course_id'])->delete();
                        $course->delete();
                ...
                if (App::auth($info['info_user'], 1)) {
                    ...
                    if ($course) {
                        if (Validate::name($res, $request->all(), 'name', 'Tên sách')) {
                            // tiến hành tạo :))
                            $session = new ladvise_session;
                            $session->name = $request->name;
                            $session->slug = Str::slug($request->name, '-');
                            $session->ladvise_id = $request_all['course_id'];
                            if (!empty($request->description)) {
